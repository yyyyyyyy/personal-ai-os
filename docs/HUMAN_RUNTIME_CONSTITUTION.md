...
宪法的意义，是绑住未来那个想用「实现方便」挪动权力的人。

> **维护者 lint：** 这次改动是否改变了某条边界的 MUST、或 Authority Graph 中任何一方的否决权 / 行动权 / 否决权归属？
> - 否 → 是实现（§8），自由演化。
> - 是 → **需要修宪（Constitutional Amendment）**：显式 RFC + 记录被改动的权力分配 + 说明为何这不是为了便利而集中权力。
