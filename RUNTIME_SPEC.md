...
>
> 状态：**v1.0 — FROZEN（对象模型已冻结）** ｜ 适配代码基线：backend v0.8.0
>
> 冻结声明：7 个 Primitive、1 条 Boundary、1 组 ABI 已收敛完成，**不再新增 Primitive、不再加 Layer、不再扩 Runtime 概念**。
> v1.0 相对 v0.1 仅吸收三处外科级微调：①`subject` → `aggregate_type + aggregate_id`（必改）②`caused_by` 增补 `correlation_id`（推荐）③ABI 新增 `subscribe_events`（让 Event 从「日志」升级为「总线」）。
> 下一步是实现，不是改 spec。第一刀验证：**清空 goals 表数据后，仅靠 Event Log 能否完整重建。**
...
核验现有代码（`backend/app/store/database.py`）可以确认，现在的调用链是：
...
```59:67:c:\Users\zhouyao\Desktop\pythonProjects\personal-ai-os\backend\app\store\database.py
...
现有 `events` 表只是一张**业务表**：没有单调递增序号、没有全局顺序保证、可被 UPDATE/DELETE、且与 `goal` 强耦合。

要成为真正的 Event Log，需补齐：
...
```69:76:c:\Users\zhouyao\Desktop\pythonProjects\personal-ai-os\backend\app\store\database.py
...
现有 `memories` 表是扁平的存储事实，**没有 `confidence` / `decay` / `derived_from_event` 字段**，也没有自动抽取流程（`local_llm` 已实现但未接线）。目标态需补：置信度、衰减、来源事件引用。
...
> 现状：`MCPHub` 已是 Capability Runtime 的雏形（22 个工具注册 + `needs_confirmation` 标志）。
> 目标态：能力调用**必须**经 Kernel（`kernel.invoke_capability`），并自动 emit `CapabilityInvoked` 事件 + 经 Approval 裁决。
...
> 现状：`ApprovalEngine` + `needs_confirmation` 存在，但 Brain 遇确认操作直接返回 `skipped`，前端 `ConfirmationDialog` 未接线——**治理层断裂**。
> 目标态：所有高风险 Capability 调用强制过 Approval，裁决结果 emit `ApprovalGranted/Denied` 事件。
...
现在系统还不大、债务还不重，正是把它从 **Personal AI App** 扭成 **Personal AI Runtime** 的最佳时间点。
