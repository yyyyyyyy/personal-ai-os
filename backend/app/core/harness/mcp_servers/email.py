...
class EmailServer:
    ...
    def _fetch_sorted_emails(
        ...
        try:
            ...
            # Fetch a candidate pool, then sort by Date header — IMAP sequence order
            # can diverge slightly from received time; pre-slicing to `limit` alone
            # would drop newer messages in edge cases.
    ...
    def check_inbox(self, limit: int = 10, unread_only: bool = False) -> str:
        ...
        try:
            emails = self._fetch_sorted_emails(limit, unread_only, body_max=300)
            slim = [
                {k: v for k, v in em.items() if k != "body"}
                for em in emails
            ]
            return json.dumps(
                {"count": len(slim), "unread_only": unread_only, "emails": slim},
                ensure_ascii=False,
            )
